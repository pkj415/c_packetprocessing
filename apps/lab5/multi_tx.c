...
__declspec(shared cls) volatile uint64_t tx_buf_start_lc;
__declspec(shared cls) volatile struct ring_meta tx_meta;

__declspec(shared cls) volatile uint64_t shadow_head;
__declspec(local_mem) volatile uint64_t shadow_head_lc, shadow_head_prev_lc, tx_meta_len_lc;
...
void dma_packet_recv(__mem40 void *addr, uint8_t dma_len) {
    ...
    DEBUG(ctx(), 0x55555555, shadow_head_lc, dma_len);
    pcie_dma_setup(&dma_cmd,
        __signal_number(&cmpl_sig),
        dma_len,
        (uint32_t)((uint64_t)addr >> 32), (uint32_t)((uint64_t)addr & 0xffffffff));

    dma_cmd.pcie_addr_hi = shadow_head_lc >> 32;
    dma_cmd.pcie_addr_lo = shadow_head_lc & 0xffffffff;
    ...
    DEBUG(ctx(), 0x66666666, shadow_head_lc, dma_len);
    shadow_head_lc += dma_len;
    if (shadow_head_lc == (tx_buf_start_lc + tx_meta_len_lc)) {
        shadow_head_lc = tx_buf_start_lc;
    }
    DEBUG(ctx(), 0x77777777, shadow_head_lc, dma_len);
}
...
int main(void)
{
    ...
    __imem struct nbi_meta_catamaran nbi_meta_gen; // Piyush TODO - Change the memory placement of this struct
    ...
    if (ctx() != 0) {
        ...
        shadow_head = tx_meta.head;
        tx_buf_start_lc = tx_meta.head;
    }

    tx_meta_len_lc = tx_meta.len;

    pkt_off = PKT_NBI_OFFSET + MAC_PREPEND_BYTES;

    DEBUG(ctx(), 0x00000000, 0x00000000, 0x00000000);
    for (;;) {
        // Allocate a packet
        ...
        DEBUG(ctx(), 0x11111111, shadow_head, tx_meta.tail);
        while (1) {
            curr_tx_tail = tx_meta.tail;
            shadow_head_lc = shadow_head;

            // Now check for a packet in TX ring and send.
            if (shadow_head_lc < curr_tx_tail) {
                if (curr_tx_tail - shadow_head_lc >= UDP_PACKET_SZ_BYTES) {
                    break;
                }
            } else {
                if (shadow_head_lc == curr_tx_tail) {
                    ...
                    two_part_dma_first_len = (tx_buf_start_lc + tx_meta_len_lc) - shadow_head_lc;
                    two_part_dma_second_len = curr_tx_tail - tx_buf_start_lc;
        ...
        shadow_head_prev_lc = shadow_head_lc;
        shadow_head_lc += UDP_PACKET_SZ_BYTES;
        if (shadow_head_lc >= (tx_buf_start_lc + tx_meta_len_lc)) {
            shadow_head_lc = tx_buf_start_lc + (shadow_head_lc - (tx_buf_start_lc + tx_meta_len_lc));
        }

        shadow_head = shadow_head_lc;
        shadow_head_lc = shadow_head_prev_lc;

        DEBUG(__ctx(), 0x22222222, shadow_head_lc, 0x00000000);
        if (shadow_head_lc + UDP_PACKET_SZ_BYTES <= (tx_buf_start_lc + tx_meta_len_lc)) {
        ...
        DEBUG(__ctx(), 0x33333333, 0x00000000, 0x00000000);
        while (tx_meta.head != shadow_head_prev_lc) {
        }

        DEBUG(ctx(), 0x44444444, 0x00000000, 0x00000000);
        tx_meta.head = shadow_head_lc;
