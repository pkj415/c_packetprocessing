...
__declspec(shared mem) volatile struct ring_meta rx_meta;
...
main(void)
{
    ...
    for (;;) {
        ...
        if (pkt_rxed.nbi_meta.pkt_info.len != UDP_PACKET_SZ_BYTES + 2*MAC_PREPEND_BYTES) {
          // DEBUG(0x11111111, 0x11111111, UDP_PACKET_SZ_BYTES + 2*MAC_PREPEND_BYTES, pkt_rxed.nbi_meta.pkt_info.len);
          DEBUG_MEM(pkt_ctm_ptr40(pkt_rxed.nbi_meta.pkt_info.isl, pkt_rxed.nbi_meta.pkt_info.pnum, PKT_NBI_OFFSET),
               pkt_ctm_data_size(pkt_rxed.nbi_meta.pkt_info.len, PKT_NBI_OFFSET, PKT_CTM_SIZE_256));
          drop_packet(&pkt_rxed.nbi_meta);
          continue;
        }
        ...
        pkt_len = pkt_rxed.nbi_meta.pkt_info.len - pkt_ctm_data_size(pkt_rxed.nbi_meta.pkt_info.len, PKT_NBI_OFFSET, PKT_CTM_SIZE_256);
        ...
        if (pkt_len != 0) {
            ...
            if (first_dma_len != pkt_len)
                dma_packet((__mem40 char *)pkt_rxed.nbi_meta.pkt_info.muptr + first_dma_len, pkt_len-first_dma_len);
        }
